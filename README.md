...
MOEX FUTOI historical data is refreshed daily (`MOEX_FUTOI_REFRESH_SECONDS=86400`) from MOEX APIM using the local `.env` key and saved as deduped CSV rows under `historical_data/futoi/futoi.csv` (`MOEX_FUTOI_DATA_DIR`). A weekly cleanup (`MOEX_FUTOI_PRUNE_SECONDS=604800`) keeps only the latest two years (`MOEX_FUTOI_RETENTION_DAYS=730`). Docker Compose also writes those rows into Postgres through the same refresh job. Active users can read stored rows through `/api/futoi?date=YYYY-MM-DD&ticker=IMOEXF`.
