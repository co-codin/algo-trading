...
class HistoricalDataTests(unittest.TestCase):
    def test_refresh_service_updates_existing_csv_and_keeps_one_year(self):
        day_ms = 24 * 60 * 60 * 1000
        now = int(datetime(2026, 6, 16, tzinfo=timezone.utc).timestamp() * 1000)
        latest = _candle(now, 12.0)
        client = FakeHistoricalClient(
            [
                _candle(now - (366 * day_ms), 10.0),
                _candle(now - (365 * day_ms), 11.0),
                latest,
            ]
        )
        ...
        self.assertNotIn(str(now - (366 * day_ms)), "\n".join(rows))
        self.assertIn(str(now - (365 * day_ms)), "\n".join(rows))
        self.assertIn(str(now), "\n".join(rows))

    def test_prune_all_uses_current_clock_for_one_year_cutoff(self):
        day_ms = 24 * 60 * 60 * 1000
        now = int(datetime(2026, 6, 16, tzinfo=timezone.utc).timestamp() * 1000)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path.write_text(
                "open_time,open,high,low,close,volume\n"
                f"{now - (366 * day_ms)},10,11,9,10,1\n"
                f"{now - (365 * day_ms)},11,12,10,11,1\n",
                encoding="utf-8",
            )
            service = HistoricalCsvRefreshService(
                data_dir=Path(tmp),
