...
def run_backtest(candles: list[Candle], config: StrategyConfig) -> BacktestResult:
    ...
    for index, candle in enumerate(candles):
        ...
        account.on_candle(candle, fast, slow, rsi_values, index, is_last_candle)
...
class _Account:
    ...
    def on_candle(
        ...
        if self.position is not None:
            ...
            strategy_exit = exit_signal_for_position(self.position.side, fast, slow, index)
        ...
        if self.position is None and not is_last_candle:
            signal = signal_for_index(self.config, fast, slow, rsi_values, index)
